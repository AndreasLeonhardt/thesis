...
\begin{figure}
 \begin{center}
  \includegraphics[width=.9\textwidth]{../EpEm_tttU42}
...
%  \citet{PhysRevLett.108.177003} used a Heisenberg model with up to third neighbour couplings, that were fitted to their measurements.
% which is able to produce the difference in $\omega(\frac12\vecQ)$ and $\omega((\frac12,0)), but underestimates 
% the spin velocity. Their model depends on three parameters, that were not connected to the underlying physics except for the fit.
% Our value of $U$ and the couplings provides Heisenberg like couplings according to 
%\begin{IEEEeqnarray}{rCl@{\quad}rCl}
%  J &=& & J^{\prime} &=& \nonumber \\
% J^{\prime \prime} &=& &&&
% \end{IEEEeqnarray}
