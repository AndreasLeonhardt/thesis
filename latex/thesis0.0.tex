...
% - - stronger Spin-Orbit coupling might introduce new effects.
% - geometric structure: layered and therefore effective 2D systems
...
The later ones are two-fold degenerate and will be half filled without further doping.
They form the active band, which allows us in a tight binding approximation to proceed to a pure spin model. 
We use the Hubbard model, to describe our system, since we have a strongly correlated system.
The Hubbard model is the easiest way of including these interactions in a spin system.
% $t-U$ and $t-t^{\prime}-t^{\prime \prime}-U$


We solve the Hubbard model in the mean-field approach.
...
Furthermore, we use parameters  for Sr$_2$IrO$_4$ and compare the results to measurements on this material. 
We use two sets of parameters with different degrees of nearest neighbour interactions.
...
%\end{IEEEeqnarray}
%
The first term represents the mean repulsion proportional to the number density of particles with opposite spin due to the nature of the interaction.
The second term corresponds to the coupling of a staggered field $Um_s$. 
...
$\frac{-\im \eta}{\eta^2 + (\omega-E)^2} \stackrel{\eta \rightarrow 0}{\longrightarrow} \pi \delta(\omega -E)$.
...
% meaning of poles: (position and) width
%Retarded response function, Wick rotation. 
%$\chi^{+-},\chi^{zz}$, why is %\chi^{zz} not important?
%evaluating infinite frequency sums -> appendix. 
...
\subsection{Specifications} %of the Calculation}

\subsection{Magnetization} % And Filling Factor
...
We have chosen a widely used path for in the Brillouin zone $\vec q$ that covers the main features of these systems\todo{references to papers using the path and desctiption of path. 
}.
